...
import { fetchApiData, fetchDeletedPlots, checkSupabaseConnection } from "../services/dataService"
...
const mapboxToken =
    ...
    "pk.eyJ1IjoiZnJhbmNpc2NvMTUwODAzIiwiYSI6ImNtODdjdTRoODA3Mmoyam9iM2twODBjN2sifQ.8b_Uf9VFJws6VCRWpQLl5w"
...
// Elimina el uso de style jsx
const supabaseErrorBannerStyle = {
...
const Mapa: React.FC = () => {
    ...
    const [dataFetchedOnLogin, setDataFetchedOnLogin] = useState<boolean>(false) // Nuevo estado para controlar si ya se cargaron los datos
    ...
    // Verificar si hay usuario y conexión a Supabase
    useEffect(() => {
        if (!user && !loading) {
            ...
            const verifySupabaseConnection = async () => {
                ...
                if (!isConnected) {
                    setSupabaseError("No se pudo conectar a la base de datos. Verifica tus credenciales de Supabase.")
                } else {
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            setSupabaseError(null) // Reset Supabase error state
            ...
            try {
                ...
                console.log("Parcelas eliminadas cargadas:", deletedPlotsData?.length || 0)
    ...
    // Efecto para cargar datos iniciales UNA SOLA VEZ después del inicio de sesión
    useEffect(() => {
        // Solo cargar datos si hay un usuario y aún no se han cargado los datos
        if (user && !dataFetchedOnLogin) {
            const initialLoad = async () => {
                setLoading(true)
                setError(null) // Reset error state
                console.log("Carga inicial de datos después del inicio de sesión (UNA SOLA VEZ)...")
                ...
                try {
                    ...
                    // Marcar que los datos ya se cargaron después del inicio de sesión
                    setDataFetchedOnLogin(true)
                } catch (e) {
            ...
            // Ejecutar carga inicial inmediatamente después del inicio de sesión
            initialLoad()
        }
    }, [fetchData, loading, user, initialDataLoaded, dataFetchedOnLogin])
    ...
    // Añade esta función después de handleRefresh
                    ...
                    <div className="section">
                        <div className="section-header">
                            ...
                            <div className="section-actions">
                                ...
                                <div className="last-update">Última actualización: {formatLastUpdateTime(lastUpdate)}</div>
                            </div>
                        </div>

                        {supabaseError && (
                            <div style={supabaseErrorBannerStyle}>
                                <p>{supabaseError}</p>
                                <p>
                                    Verifica que las variables de entorno REACT_APP_SUPABASE_URL y REACT_APP_SUPABASE_ANON_KEY estén
                ...
                return (
                    <div className="section">
                        ...
                        {supabaseError ? (
                            <div style={supabaseErrorBannerStyle}>
                                <p>{supabaseError}</p>
                                <p>No se pueden mostrar los datos históricos sin conexión a la base de datos.</p>
                            </div>
                        ) : (
                            <HistoricalCharts />
                        )}
                ...
                return (
                    <div className="section">
                        ...
                        {supabaseError ? (
                            <div style={supabaseErrorBannerStyle}>
                                <p>{supabaseError}</p>
                                <p>No se pueden mostrar las parcelas eliminadas sin conexión a la base de datos.</p>
                            </div>
                        ) : (
                            <DeletedPlots />
                        )}
